...
module.exports = Fetcher;
  ...
  /**
    I am really not happy about the current situation with these events
    especially for testing purposes.

    so atm we have next(), doWork(), processNextQueueItem() perhaps we do need
    multiple functions to keep them short contained units of work, but i feel the
    naming and what they contain is off.

    next kind of makes sense, but I'm not sure how much shit it should be doing,
    especially regarding error controls.

    i guess it should be responsible for setting up future calls to itself, but
    i'm not sure how much more than that it should be doing.

    doWork and processNextQueueItem might be able to be merged, and then the
    error logic separated out.

    the main point i want to drive home is that we need 100% coverage of the
    fetcher object, so we need a way to abstract some of this shit out in a
    very testable kind of way

    also i think restler variable should be on the fetcher object, in this way
    we can override it's functionality in tests so we can provide mocks etc

    end rant
  **/
    // @todo refactor complete method to update the queueItem
      // if complete isn't set, then we should probably remove it from any
      // inprogress queues, and add it to the tail (end) of the queue.
